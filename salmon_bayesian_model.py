...
with pm.Model() as salmon_model:
    ...
    sigma_alpha = pm.Uniform('sigma_alpha', 0, 5)
    
    # Параметры процесса
    phi = pm.Uniform('phi', 0, 0.99)
    beta = pm.Normal('beta', mu=0, sigma=100)
    sigma_proc = pm.Uniform('sigma_proc', 0, 2)
    
    # Речные эффекты
    alpha = pm.Normal('alpha', mu=mu_alpha, sigma=sigma_alpha, shape=R)
    q = pm.Beta('q', alpha=20, beta=5, shape=R)
    
    # State-space модель - используем pm.GaussianRandomWalk для AR(1)
    logS = pm.GaussianRandomWalk('logS', 
                                mu=alpha[:, None] + beta * env[None, :] + 
                                   phi * (pm.math.roll(logS, 1, axis=1) - alpha[:, None]),
                                sigma=sigma_proc, 
                                shape=(R, Tt),
                                init_dist=pm.Normal.dist(mu=alpha + beta * env[0], sigma=sigma_proc))
...
with salmon_model:
    # Сэмплирование
    trace = pm.sample(2000, tune=1000, chains=3, cores=1, 
                     return_inferencedata=True, random_seed=42)

# 4) Диагностика сходимости -----------------------------------------------
print("\nДиагностика сходимости:")
print(az.summary(trace, var_names=['mu_alpha', 'phi', 'beta', 'sigma_proc', 'sigma_alpha']))

# Проверка Rhat
rhat = az.rhat(trace)
print(f"\nМаксимальный Rhat: {rhat.max().max():.3f}")
...
posterior_means = [trace.posterior[param].mean().values for param in param_names]
...
for param in param_names:
    print(f"{param}: {eff_size[param].values:.0f}")

# 7.2) Автокорреляция
print("\nАвтокорреляция (первые 10 лагов):")
for param in param_names:
    autocorr = az.autocorr(trace, var_names=[param])
    print(f"{param}: {autocorr[param].values[:10]}")
